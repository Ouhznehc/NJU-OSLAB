...
static void kmt_sem_wait(sem_t* sem) {
  ...
  Log("TH#%p sem_wait: %s sem count: %d", current_task[cpu_current()]->stack, sem->name, sem->count);
  int sem_time = 0;
  while (sem->count == 0) {
    sem_time++;
...
static void kmt_sem_signal(sem_t* sem) {
  ...
  Log("TH#%p sem_signal: %s sem count: %d", current_task[cpu_current()]->stack, sem->name, sem->count);
  sem->count++;
  kmt_spin_unlock(&sem->lk);
}
